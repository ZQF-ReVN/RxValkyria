...
namespace ZQF::RxValkyria
{
	auto Pack::Export(const std::string_view msPackPath, const std::string_view msExportDir) -> bool
	{
		...
		ZxMem buffer;
		std::string path_tmp;
		for (auto& entry : index_table_mem.Span<VAL_Pack_Entry>())
		{
	...
	auto Pack::Import(const std::string_view msImportDir, const std::string_view msNewPackPath) -> bool
	{
		...
		const auto index_table_bytes = file_info_list.size() * VAL_Pack_Entry::SizeBytes();
		...
		auto& index_table_mem = cache_mem.Resize(index_table_bytes);
		{
			...
			for (auto& file_info : file_info_list)
			{
				...
				auto entry_ptr = index_table_mem.PtrCur<VAL_Pack_Entry*>();
				index_table_mem.PosInc(VAL_Pack_Entry::SizeBytes());
