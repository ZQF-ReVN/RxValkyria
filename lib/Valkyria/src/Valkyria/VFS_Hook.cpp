...
namespace Valkyria::VFS
{
    ...
    {
        if (Rut::RxSys::FileExist(cpPackPrefix))
        {
            Rut::RxCmd::PutFormat("ReadScript:%s [Hooked]\n", cpPackPrefix);
            return false;
        }

        bool status = sg_fnPackReadScript(cpPackPrefix, pInfoBuffer, pScriptCount, cpScriptSuffix, nReadMod);
        Rut::RxCmd::PutFormat("ReadScript:%s , ScriptCount:%d\n", cpPackPrefix, *pScriptCount);
        return status;
    }
    ...
    void HookScriptRead(uint32_t fnPackReadScript, bool isDebugInfo)
    {
        ...
        if (isDebugInfo)
        {
            Rut::RxHook::Detours::AttrachDirectly(&sg_fnPackReadScript, PackReadScript_Hook_Debug);
        }
        else
        {
            Rut::RxHook::Detours::AttrachDirectly(&sg_fnPackReadScript, PackReadScript_Hook);
        }
    }
    ...
    void HookMediasRead(uint32_t fnPackReadMedias, bool isDebugInfo)
    {
        ...
        if (isDebugInfo)
        {
            Rut::RxHook::Detours::AttrachDirectly(&sg_fnPackReadMedias, PackReadMedias_Hook_Debug);
        }
        else
        {
            Rut::RxHook::Detours::AttrachDirectly(&sg_fnPackReadMedias, PackReadMedias_Hook);
        }
    }
}